...
export class Notifier {
    ...
    playSound = async () => {
        if (!this.config.musicPath)
            return logger.debug("Müzik yolu bulunamadı, ses bildirimi atlanıyor");
        try {
    ...
    sendWebhook = async () => {
        if (!this.config.webhookURL)
            return logger.debug("Webhook URL bulunamadı, webhook bildirimi atlanıyor");
        try {
            ...
            if (!this.status)
                embed.addFields({ name: "Lütfen captcha'yı şu zamana kadar çözün: ", value: this.unixTime });
            webhook.send({
    ...
    sendDM = async () => {
        if (!this.config.adminID)
            return logger.debug("Admin ID bulunamadı, DM bildirimi atlanıyor");
        const admin = this.message.client.users.cache.get(this.config.adminID);
        if (!admin)
            return logger.debug("Admin bulunamadı, DM bildirimi atlanıyor");
        try {
            if (!admin.dmChannel)
    ...
    callDM = async () => {
        if (!this.config.adminID)
            return logger.debug("Admin ID bulunamadı, arama bildirimi atlanıyor");
        const admin = this.message.client.users.cache.get(this.config.adminID);
        if (!admin)
            return logger.debug("Admin bulunamadı, DM bildirimi atlanıyor");
        try {
