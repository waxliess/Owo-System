...
const sendCommand: Commands = {
    ...
    execute: (agent, message, ...args) => {
        try {
            if (message.channel.type == "DM" || message.channel.type == "GROUP_DM") return message.reply("Bu komutu bir sunucuda göndermelisiniz")
            if (!args || args.length < 2) return message.reply("Birini etiketlemeli ve gönderilecek cowoncy miktarını yazmalısınız")
            const target = message.mentions.members?.first()
            const owo = message.guild?.members.cache.get(agent.owoID)
            if (!target) return message.reply("Cowoncy göndermek için bir kullanıcı etiketlemelisiniz")
