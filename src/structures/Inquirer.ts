...
export class ConfigManager {
    ...
    private musicNotify = (cache?: string) => {
        ...
        return input({
            ...
            validate: (path) => {
                if (!fs.existsSync(path)) return "Dosya mevcut değil veya okunamıyor"
        ...
        return select<string>({
            ...
            choices: [
                ...
                ...(() => {
                    ...
                    if (!subs.length) return [{ name: "Desteklenen müzik dosyası veya dizin bulunamadı", value: dir, disabled: true }]
                    return subs.map(sub => {
    ...
    private getAdminID = (cache?: string) => {
        ...
        return input({
            ...
            message,
            validate: async (id) => {
                if (!/^\d{17,19}$/.test(id)) return "Geçersiz Kullanıcı ID'si"
                if (this.config.wayNotify.includes("call") || this.config.wayNotify.includes("dms")) {
                    if (id == this.agent.user?.id) return "Selfbot ID'si Call/DMs seçeneği için geçerli değildir"
    ...
    private editConfig = async () => {
        ...
        if (this.config.autoGem) this.config.autoCrate = await this.trueFalse("Otomatik olarak Gem Kasa kullan", this.cache?.autoCrate);
        if (this.config.autoGem) this.config.autoFCrate = await this.trueFalse("Otomatik olarak Fabled Kasa kullan", this.cache?.autoFCrate);
    ...
    public collectData = async () => {
        ...
        if (Object.keys(this.rawData).length === 0) {
            const confirm = await this.trueFalse(
                "Telif hakkı 2024-2025 © Eternity_VN [Kyou Izumi] x aiko-chan-ai [Elysia]. Tüm hakları saklıdır." 
                + "\nauranest'sunucu için  yapılmıştır,bewrq size bayram hediyesi olarak sunulmuştur. apğüdpğasdpğas al kullanın mucuk" 
        ...
        if (!this.cache) await this.editConfig()
            ...
            case "delete":
                ...
                } else logger.warn("Bu hesap için mevcut yapılandırma bulunamadı, silme işlemi atlandı")
                process.exit(0)
        }
